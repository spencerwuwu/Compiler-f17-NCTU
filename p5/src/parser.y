...
int wrt_fp = 0;
__BOOLEAN func_decl;
__BOOLEAN i2f_flag = __FALSE;
__BOOLEAN i2f_ed = __FALSE;
__BOOLEAN oper_onstack = __FALSE;
__BOOLEAN read_flag = __FALSE;
char wrt_name[256];
...
simple_stmt		: var_ref OP_ASSIGN boolean_expr MK_SEMICOLON
			...
			| PRINT 
			{
				print_heading_codegen();
			} 
			boolean_expr MK_SEMICOLON { 
				verifyScalarExpr( $3, "print" ); 
			    print_codegen( symbolTable, $3, scope );
			}
 			| READ 
			{
				read_flag = __TRUE;
			...
			boolean_expr MK_SEMICOLON { 
			    ...
			    read_codegen( symbolTable, $3, scope );
			}
			;
...
IF_header	: IF_ condition THEN 
		  {
			ifwhile_heading_codegen( labelTable );
		  }
		  ;
...
IF_		: IF
		  {
			...
			write_label_starter( labelTable );
		  }
		;
...
while_halfer	: while_header
			  opt_stmt_list
			  ;

while_header	: WHILE_ condition_while DO
			 {
				ifwhile_heading_codegen( labelTable );
			 }
			 ;
...
WHILE_	: WHILE
	   {
		  	struct LabelNode *node = createLabel ( linenum, "while", __FALSE );
			insertLabel( labelTable, node );
			write_label_starter( labelTable );
	   }
	   ;
...
for_stmt		: FOR ID 
			{ 
			  ...
			  insertLoopVarIntoTable( symbolTable, $2, var_tmp );
			...
			{
			  verifyLoopParam( $5, $7 );
			  for_heading_codegen( labelTable, $2, $5, $7, symbolTable );
			}
			  DO
			  opt_stmt_list
			  END DO
			{
			  for_ending_codegen( labelTable, $2, symbolTable );
			  removeLabel( labelTable );
...
factor			: var_ref
			...
			| OP_SUB ID MK_LPAREN opt_boolean_expr_list MK_RPAREN
			{
			  ...
			  $$->beginningOp = SUB_t;
			  function_call_codegen( $2, $4, symbolTable, scope );
			  neg_codegen( $$ );
			}
			| literal_const
			{
